...
function generatePaymentDropdown(userId) {
  ...
  months.forEach((month) => {
    get(ref(db, `users/${userId}/paymentHistory/mma/${month}`)).then((sn) => {
      if (!sn.exists()) {
...
function displayUserData(userId) {
  ...
  get(userRef)
    .then((userSnapshot) => {
      ...
      const userCost = userData.cost.mma; // Assuming 'kick' subscription cost status
      ...
      Object.keys(users).forEach((userId) => {
        if (snapshot.val()[userId]["dir"].includes("mma")) {
          ...
          if (users[userId]["paymentHistory"]) {
            if (
              users[userId]["cost"] &&
              users[userId]["cost"]["mma"] &&
              userCost !== "free"
            ) {
              if (
                users[userId]["paymentHistory"] &&
                users[userId]["paymentHistory"]["mma"]
              ) {
              ...
              if (
                ...
                if (
                  !Array.from(
                    Object.keys(users[userId]["paymentHistory"]["mma"])
            ...
            if (userCost === "paid") {
              if (
                ...
                if (users[userId]["paymentHistory"]) {
                  if (users[userId]["paymentHistory"]["mma"]) {
                    cardClass += " y";
                  }
                } else {
                  cardClass += " y";
